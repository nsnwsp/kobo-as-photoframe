...
if [ "$current_hour" -eq "$desired_hour" -a "$current_minute" -eq "$desired_minute" ]; then
    ...
    wget *picture url* -O /mnt/onboard/.photoframe/downloaded.png && mv /mnt/onboard/.photoframe/downloaded.png /mnt/onboard/.kobo/screensaver/current.png  && echo "pic downloaded and moved" >> /mnt/onboard/.photoframe/logd.txt
fi
